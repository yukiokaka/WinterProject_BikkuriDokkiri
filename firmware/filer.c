...
{
	if (sz < 10000) {
		xfprintf(disp_putc, "%7lu", sz);
	} else if (sz < 100 * 1024) {
		xfprintf(disp_putc, "%5lu.%luK", sz / 1024, (sz % 1024) * 10 / 1024);
	} else if (sz < 10000 * 1024) {
		xfprintf(disp_putc, "%7luK", sz / 1024);
	} else {
		xfprintf(disp_putc, "%5lu.%luM", sz / 1048576, (sz % 1048576) * 10 / 1048576);
	}
}
...
/*------------------------------------------*/
/* Open a file in associated method         */
...
)
{
	...
	if (f_open(&fil, fn, FA_READ) == FR_OK) {
		if (strstr_ext(fn, ".BMP")) {	/* BMP image viewer (24bpp, 1280 pix in max width) */
		...
		load_txt(&fil, work, sz_work);	/* Text viewer (defult) */
		f_close(&fil);
	}
}
